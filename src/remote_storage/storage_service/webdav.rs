...
// reqwest reports TLS handshake failures with is_connect() == true (the
// handshake happens during connect). Walking the source chain for TLS/cert
// keywords is the only reliable way to distinguish a genuine TCP refusal
// from a cert verification failure — and the user remedy is very different.
fn looks_like_tls_error(e: &reqwest::Error) -> bool {
    ...
    match inner_error {
        reqwest_dav::Error::Reqwest(e) => {
            ...
            if e.is_timeout() {
                ...
                if looks_like_tls_error(&e) {
                    error::Error::RemoteStorageCallError(format!(
                        "TLS handshake failed. If the server uses a self-signed certificate, enable 'Allow Untrusted Cert' on the connection entry."
                    ))
                } else {
